...
CLONE_VOICE_AGENT_PARAMETERS = {
    ...
    "required": ["audio_url", "text_to_synthesis", "is_authorized_to_clone_voice"],
}
...
class CloneVoiceAgent(BaseAgent):
    ...
    def run(
        ...
        """
        try:
            self.output_message.actions.append("Checking if you are authorised to clone the voice..")
            text_content = TextContent(
                agent_name=self.agent_name,
                status=MsgStatus.progress,
                status_message="Checking authorisation..",
            )
            self.output_message.content.append(text_content)
            self.output_message.push_update()

            if not is_authorized_to_clone_voice:
                text_content.text = "Not authorised!"
                text_content.status = MsgStatus.error
                text_content.status_message = "Not authorised"
                self.output_message.publish()
                return AgentResponse(status=AgentStatus.ERROR, message="Not authorised to clone the voice")

            text_content.text = "authorised!"
            text_content.status = MsgStatus.success
            text_content.status_message = "authorised"
            self.output_message.publish()
        except Exception as e:
            logger.exception(f"Error in {self.agent_name}")
            text_content.status = MsgStatus.error
            text_content.status_message = "Error in cloning the voice"
            self.output_message.publish()
